...
inline
void mul_add_mixed(uint32_t* out,
                   uint64_t* out_carry,
                   uint32_t a,
                   uint64_t b,
                   uint32_t c,
    ...
    umaal96(o0, o1, o2, a, b);
    *out = o0;
    *out_carry = ((uint64_t)o2 << 32) + (uint64_t)o1;
    return;
}
...
{
    uint64_t k = t[0] * inv;
    uint32_t _, carry0=0, carry1=0;
    uint32_t* r32 = (uint32_t*)r;
    const uint32_t* t32 = (const uint32_t*)t;
    const uint32_t* m32 = (const uint32_t*)modulus;

    umaal96(_,       carry0, carry1, m32[0],  k, t32[0]);
    umaal96(_,       carry0, carry1, m32[1],  k, t32[1]);
    umaal96(r32[2],  carry0, carry1, m32[2],  k, t32[2]);
    umaal96(r32[3],  carry0, carry1, m32[3],  k, t32[3]);
    umaal96(r32[4],  carry0, carry1, m32[4],  k, t32[4]);
    umaal96(r32[5],  carry0, carry1, m32[5],  k, t32[5]);
    umaal96(r32[6],  carry0, carry1, m32[6],  k, t32[6]);
    umaal96(r32[7],  carry0, carry1, m32[7],  k, t32[7]);
    umaal96(r32[8],  carry0, carry1, m32[8],  k, t32[8]);
    umaal96(r32[9],  carry0, carry1, m32[9],  k, t32[9]);
    umaal96(r32[10], carry0, carry1, m32[10], k, t32[10]);
    umaal96(r32[11], carry0, carry1, m32[11], k, t32[11]);

    add64(&r[6], &r[7], a, t[6], carry0 + (((uint64_t)carry1)<<32));
}
...
extern "C" void c_mul(uint64_t* output, const uint64_t* left, const uint64_t* right) {
    ...
    // cast down to 32 bit mode, which is ~50% faster on arm32
    mul32x12((uint32_t*)tmp, (const uint32_t*)left, (const uint32_t*)right);

    // slower with 64 bit math:
    //mul64x6(tmp, left, right);
